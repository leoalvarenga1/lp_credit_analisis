...
try {
  const fileContent = fs.readFileSync('figma_data_raw.json', 'utf8');
  ...
  if (data.status === 404) {
      console.error("Figma API Error: 404 Not Found. The file URL/ID might be incorrect.");
      process.exit(1);
  }
  
  console.log("Figma Data Loaded. Document Name:", data.name);
  ...
  if (data.document) {
      ...
      console.log("No document found in JSON, raw data keys:", Object.keys(data));
  }
